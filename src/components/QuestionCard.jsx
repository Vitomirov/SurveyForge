...
import { Toggle, IconBtn } from './ui/Ui.jsx'
import { ChoiceEditor }        from './editors/ChoiceEditor.jsx'
import { OpenTextEditor }      from './editors/OpenTextEditor.jsx'
import { DateEditor }          from './editors/DateEditor.jsx'
import { MatrixEditor }        from './editors/MatrixEditor.jsx'
import { BipolarMatrixEditor } from './editors/BipolarMatrixEditor.jsx'
import { MaxDiffEditor }       from './editors/MaxDiffEditor.jsx'
import { CardSortEditor }      from './editors/CardSortEditor.jsx'
import { ConstantSumEditor }   from './editors/ConstantSumEditor.jsx'
import { SliderEditor }        from './editors/SliderEditor.jsx'
import { NpsEditor }           from './editors/NpsEditor.jsx'
import { StarRatingEditor }    from './editors/StarRatingEditor.jsx'
import { RankingEditor }       from './editors/RankingEditor.jsx'
import { TextboxListEditor }   from './editors/TextboxListEditor.jsx'
import { SemanticDiffEditor }  from './editors/SemanticDiffEditor.jsx'
import { CascadeEditor }       from './editors/CascadeEditor.jsx'
import { ImageChoiceEditor }   from './editors/ImageChoiceEditor.jsx'
import { VisibilityEditor }    from './VisibilityEditor.jsx'
import { makeToken, tokenLabel, isPipeableSource } from '../utils/piping.js'
import { getTypeMeta, TYPE_COLORS, isChoiceType, QUESTION_TYPES } from '../utils/questionHelpers.js'
import { makeOption, makeMatrixRow, makeMatrixCol } from '../store/surveyStore.js'
        ...
        {isActive && (
          <div className="border-t border-ink-100 px-4 pt-4 pb-5 space-y-4">
            ...
            {isChoiceType(question.questionType) && (
              <ChoiceEditor question={question} dispatch={dispatch} focusOptionId={focusOptionId} availableQuestions={availableQuestions} />
            )}
            {question.questionType === 'open_text' && (
              <OpenTextEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'date' && (
              <DateEditor question={question} dispatch={dispatch} surveyDateFormat={surveyDateFormat} />
            )}
            {question.questionType === 'matrix' && (
              <MatrixEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'bipolar_matrix' && (
              <BipolarMatrixEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'maxdiff' && (
              <MaxDiffEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'card_sort' && (
              <CardSortEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'constant_sum' && (
              <ConstantSumEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'slider' && (
              <SliderEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'nps' && (
              <NpsEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'star_rating' && (
              <StarRatingEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'ranking' && (
              <RankingEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'textbox_list' && (
              <TextboxListEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'semantic_diff' && (
              <SemanticDiffEditor question={question} dispatch={dispatch} />
            )}
            {question.questionType === 'cascade' && (
              <CascadeEditor question={question} dispatch={dispatch} />
            )}
            {(question.questionType === 'image_choice_single' || question.questionType === 'image_choice_multi') && (
              <ImageChoiceEditor question={question} dispatch={dispatch} />
            )}
