...
import { ruleIsConfigured, ruleSummaryPhrase } from '@/utils/survey/conditions/ruleVariant'
...
export function analyzeBranches(items) {
  ...
  questions.forEach(q => {
    const rules = (q.terminationRules || []).filter(r => ruleIsConfigured(r, q))
    ...
    branches.push({
      ...
      triggerRuleId:    rule.id,
        triggerDesc:      ruleSummaryPhrase(rule, q),
    })
  })
